module NginxStage
  ...
  module PunConfigView
    ...
    def missing_home_directory?
      # FASRC modified to avoid false positive on root_squash filesystems
      # FIXME: this test should be done as the actual user, not root, and the corrected fix should be contributed upstream
      if ( Etc.getgrnam('rc_admin').mem.include? user.name )  || ( Etc.getgrnam('cohen_lab').mem.include? user.name )
        false
      else
        ! Dir.exist?(user.dir)
      end
